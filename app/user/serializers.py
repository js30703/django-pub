...
from app.user.examples import login_examples
from app.authentication.models import User, Social, SocialKindChoices, Device
...
class UserSocialLoginSerializer(serializers.Serializer):
    ...
    def get_facebook_user_id(self, code, redirect_uri):
    ...
    def get_google_user_id(self, code, redirect_uri):
    ...
    def get_apple_user_id(self, code, redirect_uri):
...
class UserRegisterSerializer(serializers.Serializer):
    ...
    device_token = serializers.CharField(write_only=True, required=False, allow_blank=True, allow_null=True, help_text='모바일앱에서만 사용하며 fcm token값입니다.')
...
class UserDeviceUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        try:
            attrs['device'] = self.instance.device_set.get(token=attrs['uid'])
    ...
    def update(self, instance, validated_data):
        ...
        if device:
            device.token = validated_data['token']
            device.save()
        else:
            Device.objects.create(user=self.instance, uid=validated_data['uid'], token=validated_data['token'])
        return {}
